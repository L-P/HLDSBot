...
var testVaultZIPExpectedFailures = map[string]error{
...
func TestVaultZIP(t *testing.T) {
	...
	for _, path := range paths {
		...
		if err != nil {
			if expected, ok := testVaultZIPExpectedFailures[path]; ok {
				require.ErrorIs(t, err, expected)
				continue
			}
			require.NoError(t, err, "can parse zip without error: %s", path)
		}

		require.NoError(t, ma.Close(), "can close zip without error %s", path)
	}
}
